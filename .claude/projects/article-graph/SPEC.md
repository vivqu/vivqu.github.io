...
**Venn-aware subcluster seeding:** The seed *angle* assigned to each subcluster is not arbitrary — subclusters within a supercluster are first **seriated by shared-node count** so that Venn-overlapping ones (sharing nodes) get adjacent angles and a hub subcluster ends up seated *between* its partners, while disjoint ones land opposite. A node that belongs to several subclusters is then seeded at the **average** of their directions, i.e. inside the overlap lens, rather than dumped into just one. This matters because assigning angles by iteration order can put two subclusters that both overlap a third on opposite sides of the supercluster; their shared nodes then get dragged violently across the whole supercluster on the first ticks, and the supercluster circle briefly balloons to 2–4× its settled size before recovering (observed on the `ecosystem` cluster, where `open-source` is overlapped by both `security` and `economics`). Seeding the overlap correctly removes that transient at the root — no off-screen pre-warming needed, the layout stays fully dynamic.
...
**Velocity cap:** On tick 1, `forceLink` computes velocities proportional to the distance between linked nodes. For distantly-seeded pairs this can be enormous — enough to send nodes flying to the canvas boundary, where the position clamp kills one velocity axis while the other keeps accumulating, producing axis-aligned chains of nodes. A per-tick velocity cap prevents this without affecting the long-term equilibrium. As a second guard, when the position clamp *does* pin a node to a wall, its velocity is zeroed — a node only reaches the wall in a runaway, and killing the velocity stops it sliding along the edge (the sliding is what accumulates into the chains). Both are backstops; the actual cause of early blow-ups is seed geometry (see Venn-aware seeding above), which is where it's fixed.

### Tuning knobs

Dials in `_includes/reading-graph.html`:

- **`#graph-container` `max-width`** — physical width of the graph box on the page (height auto-fills the viewport via `updateContainerHeight`).
- **`MIN_VW` / `MIN_VH`** — virtual canvas floor; bigger = clusters spread farther apart and load more zoomed out, smaller = clusters fill the frame (don't go so small they crowd the walls).
- **seed ring (`VW * 0.27`) + `minDist` gap in `superclusterProximityForce`** — space between superclusters; raise both together.
- **`spreadR` (seeding loop)** — how tightly nodes start within a subcluster; adjust gently, and note it can't change the force-determined settled radius.
- **initial `alpha`** — energy of the first ticks; lower softens the opening so big clusters don't pop oversized.
- **`alphaDecay`** — settle speed; lower = slower, more gradual (below ~`0.004` it never quite stops).
- **`velocityDecay`** — per-tick friction; higher = gentler motion.
